...
class Metaclass_ExecuteTrajectory_Goal(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('ros2_kdl_package')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'ros2_kdl_package.action.ExecuteTrajectory_Goal')
            logger.debug(
...
class Metaclass_ExecuteTrajectory_Result(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'ros2_kdl_package.action.ExecuteTrajectory_Result')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class Metaclass_ExecuteTrajectory_Feedback(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'ros2_kdl_package.action.ExecuteTrajectory_Feedback')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class ExecuteTrajectory_Feedback(metaclass=Metaclass_ExecuteTrajectory_Feedback):
    ...
    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        if 'position_error' not in kwargs:
            self.position_error = numpy.zeros(3, dtype=numpy.float64)
        else:
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if any(self.position_error != other.position_error):
            return False
        return True
    ...
    def position_error(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.float64, \
                "The 'position_error' numpy.ndarray() must have the dtype of 'numpy.float64'"
            assert value.size == 3, \
                "The 'position_error' numpy.ndarray() must have a size of 3"
        ...
        if __debug__:
            ...
            assert \
                ...
                "The 'position_error' field must be a set or sequence with length 3 and each value of type 'float' and each double in [-.000000, .000000]"
        self._position_error = numpy.array(value, dtype=numpy.float64)
...
class Metaclass_ExecuteTrajectory_SendGoal_Request(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            from ros2_kdl_package.action import ExecuteTrajectory
...
class ExecuteTrajectory_SendGoal_Request(metaclass=Metaclass_ExecuteTrajectory_SendGoal_Request):
    ...
    _fields_and_field_types = {
        ...
        'goal': 'ros2_kdl_package/ExecuteTrajectory_Goal',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.NamespacedType(['unique_identifier_msgs', 'msg'], 'UUID'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['ros2_kdl_package', 'action'], 'ExecuteTrajectory_Goal'),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_Goal
...
class Metaclass_ExecuteTrajectory_SendGoal(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'ros2_kdl_package.action.ExecuteTrajectory_SendGoal')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from ros2_kdl_package.action import _execute_trajectory
...
class ExecuteTrajectory_SendGoal(metaclass=Metaclass_ExecuteTrajectory_SendGoal):
    from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_SendGoal_Request as Request
    from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_SendGoal_Response as Response
...
class ExecuteTrajectory_GetResult_Response(metaclass=Metaclass_ExecuteTrajectory_GetResult_Response):
    ...
    _fields_and_field_types = {
        'status': 'int8',
        'result': 'ros2_kdl_package/ExecuteTrajectory_Result',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.BasicType('int8'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['ros2_kdl_package', 'action'], 'ExecuteTrajectory_Result'),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_Result
...
class Metaclass_ExecuteTrajectory_GetResult(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'ros2_kdl_package.action.ExecuteTrajectory_GetResult')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class ExecuteTrajectory_GetResult(metaclass=Metaclass_ExecuteTrajectory_GetResult):
    from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_GetResult_Request as Request
    from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_GetResult_Response as Response
...
class Metaclass_ExecuteTrajectory_FeedbackMessage(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'ros2_kdl_package.action.ExecuteTrajectory_FeedbackMessage')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
...
class ExecuteTrajectory_FeedbackMessage(metaclass=Metaclass_ExecuteTrajectory_FeedbackMessage):
    ...
    _fields_and_field_types = {
        'goal_id': 'unique_identifier_msgs/UUID',
        'feedback': 'ros2_kdl_package/ExecuteTrajectory_Feedback',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.NamespacedType(['unique_identifier_msgs', 'msg'], 'UUID'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['ros2_kdl_package', 'action'], 'ExecuteTrajectory_Feedback'),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        from unique_identifier_msgs.msg import UUID
        self.goal_id = kwargs.get('goal_id', UUID())
        from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_Feedback
...
class ExecuteTrajectory(metaclass=Metaclass_ExecuteTrajectory):
    ...
    from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_Goal as Goal
    # The result message defined in the action definition.
    from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_Result as Result
    # The feedback message defined in the action definition.
    from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_Feedback as Feedback

    class Impl:

        # The send_goal service using a wrapped version of the goal message as a request.
        from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_SendGoal as SendGoalService
        # The get_result service using a wrapped version of the result message as a response.
        from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_GetResult as GetResultService
        # The feedback message with generic fields which wraps the feedback message.
        from ros2_kdl_package.action._execute_trajectory import ExecuteTrajectory_FeedbackMessage as FeedbackMessage
